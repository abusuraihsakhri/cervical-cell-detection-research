...
from evaluation.taxonomy import ordered_names, binary_index_map, native_index_map
...
def build_confusion_matrix(model_path: str, data_yaml: str, conf_thresh: float = config.DEFAULT_INFERENCE_CONF, taxonomy="native", iou_thresh=0.5):
    from ultralytics import YOLO
    import yaml

    with open(data_yaml) as f:
        data_cfg = yaml.safe_load(f)
    class_names = ordered_names(data_cfg["names"])

    model = YOLO(model_path)

    dataset_root = Path(data_cfg.get("path", Path(data_yaml).resolve().parent))
    val_img_dir = dataset_root / data_cfg.get("val", "valid/images")
    if not val_img_dir.is_dir():
        raise ValueError(f"Missing evaluation image directory: {val_img_dir}")
    val_lbl_dir = val_img_dir.parent / "labels"

    if taxonomy == "binary":
        gt_map = binary_index_map(class_names)
        pred_map = binary_index_map(model.names)
        class_names = ["Normal", "Abnormal"]
    elif taxonomy == "agnostic":
        gt_map = {i: 0 for i in range(len(class_names))}
        pred_map = {i: 0 for i in range(len(model.names))}
        class_names = ["Cell"]
    elif taxonomy == "native":
        gt_map = {i: i for i in range(len(class_names))}
        pred_map = native_index_map(model.names, class_names)
    else:
        raise ValueError(f"Unknown taxonomy {taxonomy}")
    cm = np.zeros((len(class_names) + 1, len(class_names) + 1), dtype=int)
    bg = len(class_names)

    img_files = sorted(p for p in val_img_dir.iterdir() if p.suffix.lower() in {".jpg", ".png", ".jpeg", ".bmp"})
    if not img_files:
        raise ValueError("No evaluation images")
    for img_path in img_files:
        lbl_path = val_lbl_dir / (img_path.stem + ".txt")
        if not lbl_path.is_file():
            raise ValueError(f"Missing label: {lbl_path}")
        gt_boxes, gt_classes = [], []
        if lbl_path.exists():
            for line in lbl_path.read_text().splitlines():
                ...
                if not parts:
                    continue
                if len(parts) < 5:
                    raise ValueError(f"Malformed annotation in {lbl_path}")
                cls = int(parts[0])
                coords = list(map(float, parts[1:]))
                gt_classes.append(gt_map[cls])
                if len(coords) == 4:
        ...
        pred_classes = [pred_map[int(i)] for i in result.boxes.cls.cpu().numpy()] if len(result.boxes) else []

        pairs, unmatched_pred, unmatched_gt = match_predictions_to_gt(
            pred_boxes_xyxyn, pred_classes, gt_boxes, gt_classes, iou_thresh=iou_thresh
        )
...
def per_class_precision_recall(cm: np.ndarray, class_names: list):
    ...
    for i, name in enumerate(class_names):
        ...
        metrics[name] = {"precision": precision, "recall": recall, "f1": f1, "support": int(cm[i, :].sum())}
    return metrics
