...
changes localization metrics. Differences are descriptive unless accompanied by paired uncertainty.
...
class ReinhardNormalizer:
    ...
    def fit(self, reference_images: list):
        ...
        for p in reference_images:
            ...
            if img is None:
                raise ValueError(f"Unreadable reference image: {p}")
            lab = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
            mask = (lab[:, :, 0] < 240) & (lab[:, :, 0] > 10)
            if np.sum(mask) < 100:
                continue
            pixels = lab[mask].astype(np.float64)
            count += len(pixels)
            sums += pixels.sum(axis=0)
            squares += np.square(pixels).sum(axis=0)
        if not count:
            raise ValueError("No usable reference stain pixels")
        self.target_means = sums / count
        self.target_stds = np.sqrt(np.maximum(squares/count - self.target_means**2, 0)) + 1e-6
...
def main():
    ...
    report = {
        ...
        "inference": "Descriptive single-transform experiment; no universal noise floor or significance claim",
        "results": {},
    }
    ...
    for m_name, w_path in models_to_test:
        ...
        if dense_val_img.exists():
            ...
            report["results"][m_name]["SIPaKMeD_Dense"] = {
                ...
                "delta_f1": d_f1_d,
                "recall_increased": bool(d_rec_d > 0),
            }
