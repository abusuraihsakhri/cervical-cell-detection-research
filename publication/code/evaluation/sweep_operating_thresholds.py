...
Evaluates models across confidence thresholds from 0.01 to 0.70 to determine:
  ...
  3. Exploratory operating points (high sensitivity, high precision, cost-weighted).
    ...
    for img_name, info in gt_data.items():
        ...
        if len(res.boxes) > 0:
            ...
            for box, conf, cls_id in zip(boxes_xyxyn, confs, clss):
                mapped_cls = binary_index_map(model.names)[int(cls_id)]
                preds.append({
                    "bbox": tuple(box),
                    "conf": float(conf),
                    "cls": int(mapped_cls),
...
def find_optimal_points(sweep_results: list):
    """Extract exploratory development operating points."""
    # 1. Best F1
    ...
    return {
        ...
        "development_recall_at_least_050": best_screen_pt,
        "development_precision_at_least_095": best_confirm_pt,
...
def plot_results(all_sweeps: dict, output_png: Path):
    ...
    axD.set_ylabel("Count (Across 40 Fields)", fontsize=11, fontweight="bold")
...
def main():
    ...
    all_sweeps = {}
    summary_report = {
        "status": "EXPLORATORY: thresholds selected and scored on the same development fields; user-attested annotation review; model-assisted labels",
