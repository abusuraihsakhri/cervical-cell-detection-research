"""Sequential, resumable paired-seed training on the overlap-aware v3 splits.

Refuses to start unless the v3 evaluation protocol is frozen first.
"""
import hashlib
import json
import os
...
def main():
    manifest = ROOT / 'data/research_v3/manifest.json'
    frozen = RESULTS / 'evaluation_protocol.json'
    if not frozen.exists() or json.loads(frozen.read_text())['training_manifest_sha256'] != sha(manifest):
        raise RuntimeError('Freeze the v3 evaluation protocol against the current manifest before training')
    protocol = {
        'version': 3, 'manifest_sha256': sha(manifest), 'evaluation_protocol_sha256': sha(frozen),
        'seeds': [17, 43, 101], 'variants': ['sipakmed_only', 'combined'], 'epochs_ceiling': 150,
        ...
        'limitation': 'Retrospective reanalysis; dense-field overlap components withheld from training and checkpoint selection. No clinical claim.',
    }
    ...
    for seed in protocol['seeds']:
        for variant in protocol['variants']:
            ...
            command = [sys.executable, '-u', str(ROOT / 'training_pipeline.py'), '--data',
                       str(ROOT / 'data/research_v3' / variant / 'data.yaml'),
