...
(either as Label Studio JSON export or YOLO format), this script:
  ...
  3. Computes descriptive metrics; independent annotation provenance is required separately.
...
def parse_labelstudio_export(json_path: Path, images_dir: Path, out_dir: Path) -> Path:
    ...
    if not isinstance(data, list) or not data:
        raise ValueError("Expected a nonempty Label Studio task list")
    seen = set()
    for task in data:
        annotations = task.get("annotations", [])
        if len(annotations) != 1 or annotations[0].get("was_cancelled"):
            raise ValueError("Exactly one completed, adjudicated annotation is required per task; predictions are never ground truth")
        if not annotations[0].get("completed_by") or not annotations[0].get("created_at"):
            raise ValueError("Original reviewer and timestamp metadata required; historical exports need separately documented attestation")
        name = task.get("data", {}).get("image_name")
        if not name:
            name = Path(task.get("data", {}).get("image", "").split("=")[-1]).name
        if not name or name in seen or not (images_dir / name).is_file():
            raise ValueError(f"Missing or duplicate source image: {name}")
        seen.add(name)
        for result in annotations[0].get("result", []):
            ...
            if len(labels) != 1 or labels[0].strip().lower() not in CLASS_MAP_2CLASS:
                raise ValueError(f"Unknown or ambiguous class: {labels}")
            x, y, w, h = (float(value[k]) for k in ("x", "y", "width", "height"))
            if not (0 <= x < 100 and 0 <= y < 100 and w > 0 and h > 0 and x+w <= 100.001 and y+h <= 100.001):
                raise ValueError("Invalid rectangle geometry")
    if out_dir.exists() and any(out_dir.iterdir()):
        raise ValueError("Import into a new empty directory to avoid stale labels")
    ...
    for task in data:
        ...
        ann_results = annotations[0]["result"]
        ...
        for r in ann_results:
            ...
            cls_id = CLASS_MAP_2CLASS[label_name]
...
def evaluate_models(verified_yaml: Path, conf: float = 0.111, iou_thresh: float = 0.5):
    ...
    report = {"status": "DEVELOPMENT ONLY: historical validation fields; review confirmed by user; model-assisted labels, no independent second review", "conf_threshold": conf, "iou_threshold": iou_thresh, "models": {}}
    print("\n" + "=" * 80)
    ...
    for name, weights_path in candidate_models:
        ...
        try:
            ...
            cm, names = build_confusion_matrix(str(weights_path), str(verified_yaml), conf_thresh=conf, taxonomy="binary", iou_thresh=iou_thresh)
